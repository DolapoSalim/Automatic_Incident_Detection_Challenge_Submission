    ...
    for i in range(len(all_labels)):
        ...
        if label == 0:
            ...
            if detected_clip is None:
                ...
                onset_frame_gt = (
                    int(start_frames[onset_clip_gt])
                    if onset_clip_gt < len(start_frames)
                    else 0
                )

                det_frame = (
                    int(start_frames[detected_clip])
                    if detected_clip < len(start_frames)
                    else 0
                )
...
class Trainer:
    ...
    def train_epoch(self, epoch: int):
        ...
        for step, batch in enumerate(self.train_loader):
            ...
            try:
                ...
                print(f"[OOM] Skipping batch {step}")
            ...
            if step % 20 == 0:
                ...
                print(
                    f"[E{epoch} S{step}/{len(self.train_loader)}] "
                    f"loss={loss.item():.4f} "
                    f"lr={lr:.2e} "
                    f"time={elapsed:.1f}s"
                )
    ...
    def validate(self):
        ...
        for batch in self.val_loader:
            ...
            scores = out["scores"].detach().cpu().float().numpy()
    ...
    def save_checkpoint(self, epoch, metrics, tag="best"):
        ...
        for epoch in range(1, self.cfg["epochs"] + 1):
            ...
            if self.patience_counter >= self.cfg["patience"]:
                ...
                print(
                    ...
                    f"{self.patience_counter} epochs."
                )
